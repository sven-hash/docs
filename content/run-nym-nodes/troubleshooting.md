...
- Not using _--anounce-host_ flag while running mixnode from your local machine behind NAT.
...
All config and keys files are stored in a directory named after your `id` which you chose during the _init_ configuration with a following PATH: `$HOME/.nym/mixnodes/` where `$HOME` is a home directory of the user (your current user in this case) that launched the mixnode.
...
Easy solution would be to use **nohup** -> `` nohup`./nym-mixnode run --id NYM &  `` where `--id NYM` is the id you set during the _init_ command previously.
