...
:::note
This page details how to build the main Nym platform code. **If you want to build and run a validator, [go here](/docs/next/run-nodes/nodes/validators) instead.**
:::
...
1. the [mixnode](/docs/next/run-nodes/nodes/mixnodes): `nym-mixnode`
2. the [gateway node](/docs/next/run-nodes/nodes/gateways): `nym-gateway`
...
5. the [network requester](/docs/next/run-nodes/nodes/requester): `nym-network-requester`
6. the [network explorer api](/docs/next/run-nodes/nodes/network-explorer): `explorer-api`

The repository also contains two Typescript applications which aren't built in this process: the [Nym Wallet](docs/next/wallet) and the [Network Explorer UI](docs/next/run-nodes/nodes/network-explorer). Both of these can be built by following the instructions on their respective docs pages. 
